...
namespace DespesaViagem.Services.Services
{
    public class DespesaAlimentacaoService : IDespesasService<DespesaAlimentacao>
    {
        ...
        public async Task<Result<IEnumerable<DespesaAlimentacao>>> ObterTodasDespesas(int idUsuario)
        {
            Usuario usuario = await _usuarioRepository.ObterUsuario(idUsuario) ?? new();

            if (usuario.TipoDeUsuario == RolesUsuario.Funcionario)
                return Result.Failure<IEnumerable<DespesaAlimentacao>>("Usuário não autorizado.");
            ...
            if (!viagens.Any())
                return Result.Failure<IEnumerable<DespesaAlimentacao>>("Não existem viagens cadastradas.");

            List<int> idsViagens = viagens.Select(v => v.Id).ToList();

            IEnumerable<DespesaAlimentacao> despesas = await _despesaRepository.ObterTodos(idsViagens);


            if (despesas is null || !despesas.Any())
                return Result.Failure<IEnumerable<DespesaAlimentacao>>("Não existem despesas cadastradas!");

            return Result.Success(despesas);
        }


        public async Task<Result<IEnumerable<DespesaAlimentacao>>> ObterTodasDespesasViagem(int idViagem)
        {            
